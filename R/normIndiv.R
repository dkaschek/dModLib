...
#' Unclass a parvec
#' 
#' Usual unclass, but keep names
#' 
#' @param x object of class parvec
#' @return Named numeric with the same elements as x
#' 
...
#'
#' @param pars Named numeric or [parvec]. A parameter vector.
#' @param fixed Named numeric or [parvec]. A vector of fixed parameters
#' @param est.grid Data frame with columns `ID`, `condition`, and columns for the
#' parameters (see example)
#' @param fix.grid Data frame with columns `ID`, `condition`, and columns for the
#' parameters (see example)
#' @param ID single ID
#'
#' @return List with pars, fixed, parnames, and parnames_full
#'
#' @examples
...
#' For example in est.grid = data.table(ID = 1, init_A = "Atot", Atot= "Atot", ...)
#' 
#' @param ol objlist with potentially duplicated names
#'
...
#' est.grid <- data.table::data.table(ID = 1:2,
#'                        condition = c("A", "B"),
#'                        k1 = c("k1_A", "k1_B"),
#'                        k2 = c("k2_A", "k2_B"),
#'                        k3 = c("k3", NA),
#'                        k4 = c("k4", "k4"),
#'                        stringsAsFactors = FALSE)
#' fix.grid <- data.table::data.table(ID = 1:2,
#'                        condition = c("A", "B"),
#'                        k3 = c(NA, 3.5),
#'                        k5 = c(5.1,5.2),
#'                        k6 = c(6,6),
...
#'
#' @examples
#' pars <- data.table::data.table(ID = 1:2, newFix = c(1,2), newEst = c("a", "b"), newMix = c("a", 1))
#' est.grid <- data.table::data.table(ID = 1:2,
#'                        condition = c("A", "B"),
#'                        k1 = c("k1_A", "k1_B"),
#'                        k2 = c("k2_A", "k2_B"),
...
indiv_addLocalParsToGridList <- function(pars, gridlist, FLAGoverwrite = FALSE) {
  ...
  data.table::setcolorder(pars, joincols)
...
#' @param FLAGchisquare Add attribute "chisquare" which is initialized by 0.
#' @author Daniel Lill (daniel.lill@physik.uni-freiburg.de)
#' @md
#' @examples
...
#' Population function based on common base prediction function and grid of individuals
#' 
#'
#' @param prd0 Prediction function
#' @param est.grid Data frame with columns `ID`, `condition`, and columns for the
#' parameters
#' @param fix.grid Data frame with columns `ID`, `condition`, and columns for the
#' parameters
...
PRD_indiv <- function(prd0, est.grid, fix.grid) {
  ...
  if (!data.table::is.data.table(est.grid)) warning("est.grid was coerced to data.table (was", class(est.grid), ")")
  if (!data.table::is.data.table(fix.grid)) warning("fix.grid was coerced to data.table (was", class(fix.grid), ")")
  
  est.grid <- data.table::data.table(est.grid)
  fix.grid <- data.table::data.table(fix.grid)
  
  data.table::setkeyv(est.grid, c("ID", "condition"))
  data.table::setkeyv(fix.grid, c("ID", "condition"))
...
#' Generate population parameter transformation from base transformation and individual grids
#'
#' @param p0 Base parameter transformation function
#' @param est.grid Data frame with columns `ID`, `condition`, and columns for the
#' parameters 
#' @param fix.grid Data frame with columns `ID`, `condition`, and columns for the
#' parameters 
#'
#' @return Parameter transformation function
#' @export
#' @author Daniel Lill (daniel.lill@physik.uni-freiburg.de)
#' @md
...
#' Fast normL2 
#' 
#' Creates the objective function for a population of subjects with the same underlying model
#' and parametrization.
#'
#' @param data datalist object
#' @param prd0 base prediction function
#' @param errmodel error model
#' @param est.grid Data frame with columns `ID`, `condition`, and columns for the
#' parameters
#' @param fix.grid Data frame with columns `ID`, `condition`, and columns for the
#' parameters
#' @param times Optional vector of timepoints where `prd0` should be evaluated
#' @param attr.name Character, which attribute to asign the obj value for combination with other obj functions
...
#'
#' @param name character, state of observable name
#' @param time numeric, time point
#' @param value numeric, value
#' @param sigma numeric, uncertainty
#' @param attr.name character, the attribute name by which the value will be visible 
#' for combination of several objetive functions
#' @param condition character, the condition
#' @param prd_indiv population prediction function
#'
#' @return objective function
#' @export
#' @author Daniel Lill (daniel.lill@physik.uni-freiburg.de)
...
#'
#' @return List with est.grid and fixed.grid
...
getParGrids <- function(mytrafo, mytrafoL, mycondition.grid, SS_pars = NULL){
  ...
  if(nrow(fixed_df) > 0){
    ...
    data.table::setcolorder(fixed_df2, c("ID", "condition"))
  } else fixed_df2 <- NULL
  ...
  for(cond in myconditions){
    ...
    if(any(c(str_detect(conditrafo, "exp\\("),str_detect(conditrafo, "10\\^\\(")))) {
      conditrafo <- gsub("exp\\(", "", conditrafo) 
      conditrafo <- gsub("10\\^\\(", "", conditrafo) 
      conditrafo <- gsub("\\(", "", conditrafo) 
      conditrafo <- gsub("\\)", "", conditrafo)
    }
    if(any(c(str_detect(est_trafo, "exp\\("),str_detect(est_trafo, "10\\^\\(")))) {
      est_trafo <- gsub("exp\\(", "", est_trafo)
      est_trafo <- gsub("10\\^\\(", "", est_trafo)
  ...
  partly_fixed_df <- est_df |> dplyr::select_if(function(x) any(grepl("^-?[[:digit:]]",x))) 
  ...
  if(!is.null(fixed_df2) & !purrr::is_empty(partly_fixed_df)){
    fixed.grid <- fixed_df2 |> cbind(partly_fixed_df |> as.data.frame(stringsAsFactors = F)) |> dplyr::as_tibble() |> as.data.table()
    ...
    fixed.grid <- cbind(data.table(ID = 1:length(myconditions), condition = myconditions),
                        partly_fixed_df |> as.data.frame(stringsAsFactors = F) |> dplyr::as_tibble() |> as.data.table())
  ...
  est.grid  <- est_df |> as.data.frame(stringsAsFactors = F) |> mutate(ID = 1:length(myconditions)) |> 
    dplyr::as_tibble() |> as.data.table()
  data.table::setcolorder(est.grid, c("ID", "condition"))
